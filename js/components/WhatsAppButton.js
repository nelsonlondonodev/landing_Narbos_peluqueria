...
export class WhatsAppButton {
    ...
    /**
     * Renderiza e inserta el botón en el DOM.
     */
    render() {
        const container = document.createElement('div');
        container.id = 'whatsapp-button-root';
    ...
    getButtonHTML() {
        return `
            <a href="https://wa.me/573123462618?text=hola, ¡Te hablo desde la web de Narbos Salón!" 
               target="_blank" 
               aria-label="Chatea con nosotros en WhatsApp"
               class="whatsapp-float fixed bottom-6 right-6 z-50 group transition-transform hover:scale-110 duration-300">
                
                <!-- Tooltip Label -->
                <div class="absolute bottom-full mb-3 right-0 whitespace-nowrap bg-white text-brand-gray-dark px-4 py-2 rounded-xl shadow-lg border border-brand-light/20 font-serif font-bold text-sm hidden md:flex items-center gap-2 animate-bounce">
                    <span>¡Agenda tu cita aquí!</span>
                    <!-- Arrow -->
                    <div class="absolute right-6 -bottom-1.5 w-3 h-3 bg-white border-r border-b border-brand-light/20 transform rotate-45"></div>
                </div>
                ...
                <div class="relative flex items-center justify-center w-14 h-14 md:w-16 md:h-16 bg-[#25D366] rounded-full shadow-lg shadow-green-500/40 hover:shadow-green-500/60 transition-shadow">
                    <!-- SVG Icon -->
                    <svg class="w-8 h-8 md:w-10 md:h-10 text-white fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
