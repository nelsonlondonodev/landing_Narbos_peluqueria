 ...
 *
 * El marcado de la página trae una tarjeta escrita a mano como fallback: es lo que ve
 * el rastreador si el JS no llega, así que el componente la reemplaza en vez de
 * añadirse a ella.
 *
...
export class ReviewsMarquee {
    ...
    render() {
        ...
        if (reviews.length > 0) {
            this.track.innerHTML = reviews.map((r, i) => this._cardHTML(r, i)).join('');
        }
    ...
    /** @private */
    _cardHTML(review, index) {
        const initial = review.author ? review.author.charAt(0).toUpperCase() : 'C';
