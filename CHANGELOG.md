...
Todos los cambios notables de este proyecto se documentan en este archivo, en orden cronológico inverso (más reciente arriba).

> Este historial se centra en el *qué y por qué* de cada release. Para el estado de arquitectura actual, ver [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
*   **Migración de Videos a YouTube (Peluquería y Cortes)**: Reemplazados los videos locales por embebidos de YouTube en las páginas de Peluquería y Cortes de Pelo, actualizando el JSON-LD asociado (`f8eb68a`).
*   **Consolidación de VideoObject Schema en Nosotros**: Igual tratamiento de unificación de schema aplicado a la página Nosotros (`1a6090f`).
*   **Migración de Video en Nosotros**: Video local reemplazado por YouTube en Nosotros, con actualización de JSON-LD (`d532b02`).
*   **Nuevo YouTube Short y Badge "Reciente"**: Añadido un nuevo Short, estilo visual `badge-recent` para destacar contenido nuevo en la galería, y datos estructurados de video actualizados (`2ba63ab`).
*   **Refactor de Datos de Video**: Reestructurado `homeVideos` en `videoData.js` y mejorada la accesibilidad (ARIA) de `YouTubeGallery.js` (`ef29f0a`).
*   **Nuevo Video de Home**: Añadido nuevo video de YouTube y actualizado `VideoObject` structured data (`66382d8`).
*   **Versión**: `v2.8.18`.

## 🔄 Actualización de FAQ y Bump v2.8.17 (29 de junio de 2026)

*   **FAQ de Marcas (Home)**: Actualizada la lista de marcas referenciadas en el structured data de FAQ de la página de inicio.
*   **Versión**: `v2.8.17`.

---

## 🚀 Nuevo Artículo de Alisados Capilares y Depuración de Enlaces (June 27, 2026) - v2.8.16 🚀
*   **Publicación de Alisados Capilares**: Creado el nuevo post en [alisados-cabello-chia.html](blog/articles/alisados-cabello-chia.html) con metatags de SEO estrictos (Title y Description limitados en longitud), enlazado interno transaccional y marcado JSON-LD estructurado de tipo `BlogPosting` y `FAQPage`.
*   **Integración en el Index de Blog**: Registrada la tarjeta visual en [blog/index.html](blog/index.html) y añadidos los datos del nuevo artículo al marcado estructurado general en el head.
*   **Actualización de Indexación**: Agregada la URL limpia del artículo al mapa del sitio en [sitemap.xml](sitemap.xml).
...
*   **Observaciones y Pendientes de Diseño**:
    *   *Revisión de Títulos:* Revisar la mezcla de mayúsculas y minúsculas en el H1 y otros encabezados de [alisados-cabello-chia.html](blog/articles/alisados-cabello-chia.html).
